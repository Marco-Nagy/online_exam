...
void main() {
  ...
  () {
    test('when call forget password from online data source it should call apiManage.forgetPassword with correct body', () async{
      ...
      when(mockApiManager.forgotPassword(bodyForgetPassword)).thenAnswer((_) async =>mockedResult.data ,);
      var result = await authOnlineDataSourceImpl.forgotPassword(bodyForgetPassword);
      expect(result, isA<Success<ForgotPasswordResponse>>());
      verify(mockApiManager.forgotPassword(bodyForgetPassword)).called(1);
    });
    test('when call verify reset code  from online data source it should call apiManage.verifyResetCode with correct body', () async{
      var authOnlineDataSourceImpl = AuthOnlineDatasourceImpl(mockApiManager);
      ...
      when(mockApiManager.verifyResetCode(bodyVerifyResetCode)).thenAnswer((_) async =>mockedResult.data ,);
      var result = await authOnlineDataSourceImpl.verifyResetCode(bodyVerifyResetCode);
      expect(result, isA<Success<VerifyResetCodeResponse>>());
      verify(mockApiManager.verifyResetCode(bodyVerifyResetCode)).called(1);
    });

    test('when call reset password from online data source it should call apiManage.resetPassword with correct body', () async{
      var authOnlineDataSourceImpl = AuthOnlineDatasourceImpl(mockApiManager);
      var mockedResult = Success<AuthResponse>(AuthResponse(message,token,user));
      provideDummy<ApiResult<AuthResponse>>(mockedResult);
      when(mockApiManager.resetPassword(bodyResetPassword)).thenAnswer((_) async =>mockedResult.data ,);
      var result = await authOnlineDataSourceImpl.resetPassword(bodyResetPassword);
